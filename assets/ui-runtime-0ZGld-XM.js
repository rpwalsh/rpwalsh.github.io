var Zt=Object.defineProperty;var te=(s,e,t)=>e in s?Zt(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var a=(s,e,t)=>te(s,typeof e!="symbol"?e+"":e,t);class d extends HTMLElement{constructor(){super();a(this,"root");a(this,"_initialized",!1);this.root=this.attachShadow({mode:"open"})}connectedCallback(){this._injectTokenSheet(),this.render(),this._initialized=!0,this.onConnected()}disconnectedCallback(){this.onDisconnected()}attributeChangedCallback(t,r,o){this._initialized&&r!==o&&this.render()}onConnected(){}onDisconnected(){}styles(){return""}render(){const t=this.styles();this.root.innerHTML=(t?`<style>${t}</style>`:"")+this.template(),this.onRendered()}onRendered(){}getBoolAttr(t){return this.hasAttribute(t)}getAttr(t,r=""){return this.getAttribute(t)??r}getNumAttr(t,r=0){const o=this.getAttribute(t);return o!==null?Number(o):r}emit(t,r){this.dispatchEvent(new CustomEvent(t,{bubbles:!0,composed:!0,detail:r}))}$(t){return this.root.querySelector(t)}$$(t){return Array.from(this.root.querySelectorAll(t))}_injectTokenSheet(){const t=new CSSStyleSheet;t.replaceSync(`
    ...
    `),this.root.adoptedStyleSheets=[t]}}function l(s,e){customElements.get(s)||customElements.define(s,e)}class z extends d{styles(){return`
    ...
    `}onRendered(){var e;(e=this.$("button"))==null||e.addEventListener("click",t=>{(this.getBoolAttr("disabled")||this.getBoolAttr("loading"))&&(t.stopPropagation(),t.preventDefault())})}}a(z,"observedAttributes",["variant","size","color","disabled","loading","full-width"]);l("ui-button",z);class E extends d{get value(){const e=this.$("input");return e?e.value:this.getAttr("value")}set value(e){this.setAttribute("value",e);const t=this.$("input");t&&(t.value=e)}styles(){return`
      :host { display: block; }
    ...
    `}template(){const e=this.getAttr("variant","outlined"),t=this.getAttr("size","md"),r=this.getAttr("label"),o=this.getAttr("placeholder"),i=this.getAttr("value"),n=this.getAttr("type","text"),c=this.getAttr("helper-text"),p=this.getBoolAttr("disabled"),u=this.getBoolAttr("readonly"),h=this.getBoolAttr("required");return`
    ...
    `}onRendered(){const e=this.$("input");e&&(e.addEventListener("input",()=>{this.emit("ui-input",{value:e.value})}),e.addEventListener("change",()=>{this.emit("ui-change",{value:e.value})}))}}a(E,"observedAttributes",["variant","size","label","placeholder","value","type","helper-text","disabled","readonly","error","required"]);l("ui-text-field",E);class S extends d{get value(){const e=this.$("select");return e?e.value:this.getAttr("value")}set value(e){this.setAttribute("value",e);const t=this.$("select");t&&(t.value=e)}styles(){return`
      :host { display: block; }
      .field { display: flex; flex-direction: column; gap: 0.25rem; }
    ...
    `}template(){const e=this.getAttr("size","md"),t=this.getAttr("label"),r=this.getAttr("placeholder"),o=this.getAttr("value"),i=this.getAttr("helper-text"),n=this.getBoolAttr("disabled"),c=this.getBoolAttr("multiple"),p=Array.from(this.querySelectorAll("option")).map(u=>`<option value="${u.value}" ${u.value===o?"selected":""} ${u.disabled?"disabled":""}>${u.textContent}</option>`).join("");return`
      <div class="field" part="field">
    ...
    `}onRendered(){var e;(e=this.$("select"))==null||e.addEventListener("change",t=>{const r=t.target;this.emit("ui-change",{value:r.value})})}}a(S,"observedAttributes",["size","label","placeholder","value","helper-text","disabled","error","multiple"]);l("ui-select",S);class L extends d{get checked(){return this.getBoolAttr("checked")}set checked(e){e?this.setAttribute("checked",""):this.removeAttribute("checked")}styles(){return`
    ...
    `}template(){const e=this.getNumAttr("min",0),t=this.getNumAttr("max",100),r=this.getNumAttr("step",1),o=this.getNumAttr("value",50),i=this.getBoolAttr("disabled");return`
      <input
        ...
        part="input"
      />
    `}onRendered(){const e=this.$("input");e&&(e.addEventListener("input",()=>{this.emit("ui-input",{value:Number(e.value)})}),e.addEventListener("change",()=>{this.emit("ui-change",{value:Number(e.value)})}))}}a(B,"observedAttributes",["min","max","step","value","disabled","size","color"]);l("ui-slider",B);class I extends d{styles(){return`
      :host { display: inline-flex; }
    ...
    `}template(){return`<label><input type="radio" name="${this.getAttr("name")}" value="${this.getAttr("value")}"
      ...
      <span>${this.getAttr("label")}<slot></slot></span></label>`}onRendered(){var e;(e=this.$("input"))==null||e.addEventListener("change",t=>{const r=t.currentTarget;this.toggleAttribute("checked",r.checked),this.emit("ui-change",{checked:r.checked,value:r.value})})}}a(I,"observedAttributes",["checked","disabled","value","name","label"]);class C extends d{constructor(){super(...arguments);a(this,"handleChange",t=>{const r=t.target;if(!r.matches("ui-radio"))return;const o=r.getAttribute("value")??"";this.setAttribute("value",o),this.querySelectorAll("ui-radio").forEach(i=>i.toggleAttribute("checked",i===r)),this.emit("ui-change",{value:o})})}styles(){return`
    ...
    `}template(){return`<fieldset ${this.getBoolAttr("disabled")?"disabled":""}><legend>${this.getAttr("label")}</legend><slot></slot></fieldset>`}onConnected(){this.addEventListener("ui-change",this.handleChange)}onDisconnected(){this.removeEventListener("ui-change",this.handleChange)}}a(C,"observedAttributes",["label","value","disabled","orientation"]);l("ui-radio",I);l("ui-radio-group",C);class T extends d{styles(){return`
      :host { display:block; }
    ...
    `}template(){return`<label><span>${this.getAttr("label")}</span><input role="combobox" aria-autocomplete="list" aria-expanded="${this.open}" placeholder="${this.getAttr("placeholder")}" value="${this.getAttr("value")}" ${this.getBoolAttr("disabled")?"disabled":""}/></label><div role="listbox" ${this.open?"":"hidden"}></div>`}show(t=""){this.open=!0;const r=this.$("[role=listbox]"),o=this.options.filter(i=>`${i.label} ${i.description??""}`.toLowerCase().includes(t.toLowerCase()));r&&(r.hidden=!1,r.innerHTML=o.map((i,n)=>`<div role="option" tabindex="-1" data-value="${i.value}" aria-selected="${n===this.activeIndex}"><strong>${i.label}</strong>${i.description?`<small>${i.description}</small>`:""}</div>`).join("")||'<div role="option" aria-disabled="true">No results</div>',r.querySelectorAll("[data-value]").forEach(i=>i.addEventListener("mousedown",n=>{var c;n.preventDefault(),this.select(i.dataset.value??"",((c=i.querySelector("strong"))==null?void 0:c.textContent)??"")})))}select(t,r){this.setAttribute("value",r),this.open=!1,this.$("[role=listbox]").hidden=!0,this.emit("ui-change",{value:t,label:r})}onRendered(){const t=this.$("input");t==null||t.addEventListener("focus",()=>this.show(t.value)),t==null||t.addEventListener("input",()=>{this.activeIndex=-1,this.show(t.value),this.emit("ui-input",{value:t.value})}),t==null||t.addEventListener("blur",()=>setTimeout(()=>{this.open=!1;const r=this.$("[role=listbox]");r&&(r.hidden=!0)},80)),t==null||t.addEventListener("keydown",r=>{var i;const o=Array.from(this.root.querySelectorAll("[data-value]"));if((r.key==="ArrowDown"||r.key==="ArrowUp")&&(r.preventDefault(),this.activeIndex=Math.max(0,Math.min(o.length-1,this.activeIndex+(r.key==="ArrowDown"?1:-1))),this.show(t.value)),r.key==="Enter"&&this.activeIndex>=0&&o[this.activeIndex]){r.preventDefault();const n=o[this.activeIndex];this.select(n.dataset.value??"",((i=n.querySelector("strong"))==null?void 0:i.textContent)??"")}if(r.key==="Escape"){this.open=!1;const n=this.$("[role=listbox]");n&&(n.hidden=!0)}})}}a(R,"observedAttributes",["label","placeholder","value","disabled","options"]);l("ui-autocomplete",R);class j extends d{styles(){return":host{display:inline-flex}button{all:unset;cursor:pointer;padding:.5rem .75rem;border:1px solid var(--ui-color-border,#cbd5e1);color:var(--ui-color-text);background:var(--ui-color-surface);font:inherit;font-size:.875rem}button[aria-pressed=true]{color:var(--ui-color-primary,#4f46e5);background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 15%,transparent);border-color:var(--ui-color-primary,#4f46e5)}button:focus-visible{outline:2px solid var(--ui-color-primary,#4f46e5);outline-offset:2px}button:disabled{opacity:.5;cursor:not-allowed}"}template(){return`<button type="button" aria-pressed="${this.getBoolAttr("selected")}" ${this.getBoolAttr("disabled")?"disabled":""}><slot></slot></button>`}onRendered(){var e;(e=this.$("button"))==null||e.addEventListener("click",()=>{const t=!this.getBoolAttr("selected");this.toggleAttribute("selected",t),this.emit("ui-change",{selected:t,value:this.getAttr("value")})})}}a(j,"observedAttributes",["selected","disabled","value"]);class D extends d{constructor(){super(...arguments);a(this,"handleChange",t=>{const r=t.target;r.matches("ui-toggle-button")&&this.getBoolAttr("exclusive")&&(this.querySelectorAll("ui-toggle-button").forEach(o=>o.toggleAttribute("selected",o===r)),this.setAttribute("value",r.getAttribute("value")??""))})}styles(){return":host{display:inline-flex}:host([orientation=vertical]){display:inline-grid}::slotted(ui-toggle-button:not(:first-child)){margin-left:-1px}"}template(){return'<div role="group"><slot></slot></div>'}onConnected(){this.addEventListener("ui-change",this.handleChange)}onDisconnected(){this.removeEventListener("ui-change",this.handleChange)}}a(D,"observedAttributes",["exclusive","value","orientation"]);l("ui-toggle-button",j);l("ui-toggle-button-group",D);class q extends d{styles(){return":host{display:inline-flex}button{all:unset;display:grid;place-items:center;width:var(--_size,2.5rem);height:var(--_size,2.5rem);border-radius:50%;cursor:pointer;color:var(--_color,var(--ui-color-text));background:var(--_bg,transparent);border:1px solid var(--_border,transparent)}:host([size=sm]){--_size:2rem}:host([size=lg]){--_size:3rem}:host([variant=filled]){--_bg:var(--ui-color-primary,#4f46e5);--_color:#fff}:host([variant=outlined]){--_border:var(--ui-color-border,#cbd5e1)}button:hover{background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 12%,var(--_bg,transparent))}button:focus-visible{outline:2px solid var(--ui-color-primary,#4f46e5);outline-offset:2px}button:disabled{opacity:.45;cursor:not-allowed}"}template(){return`<button type="button" aria-label="${this.getAttr("label","Action")}" ${this.getBoolAttr("disabled")?"disabled":""}><slot></slot></button>`}}a(q,"observedAttributes",["label","size","variant","color","disabled","loading"]);l("ui-icon-button",q);class M extends d{styles(){return":host{display:inline-grid;gap:.375rem}label{font-size:.875rem;font-weight:600}.control{display:flex;border:1px solid var(--ui-color-border,#cbd5e1);border-radius:var(--ui-radius-md,.5rem);overflow:hidden;background:var(--ui-color-surface)}input{width:7rem;min-width:0;padding:.55rem .65rem;border:0;outline:0;background:transparent;color:var(--ui-color-text);font:inherit;text-align:right}button{width:2.25rem;border:0;background:transparent;color:var(--ui-color-text);cursor:pointer;font-size:1rem}button:hover{background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 10%,transparent)}button:disabled{opacity:.45;cursor:not-allowed}small{color:var(--ui-color-text-secondary,#64748b)}"}template(){return`<label>${this.getAttr("label")}</label><div class="control"><button class="dec" type="button" aria-label="Decrease" ${this.getBoolAttr("disabled")||this.getBoolAttr("readonly")?"disabled":""}>−</button><input type="number" value="${this.getNumAttr("value",0)}" min="${this.getAttr("min")}" max="${this.getAttr("max")}" step="${this.getNumAttr("step",1)}" ${this.getBoolAttr("disabled")?"disabled":""} ${this.getBoolAttr("readonly")?"readonly":""}/><button class="inc" type="button" aria-label="Increase" ${this.getBoolAttr("disabled")||this.getBoolAttr("readonly")?"disabled":""}>+</button></div><small>${this.getAttr("helper-text")}</small>`}onRendered(){var r,o;const e=this.$("input"),t=i=>{const n=this.getNumAttr("min",-1/0),c=this.getNumAttr("max",1/0),p=this.getNumAttr("precision",0),h=Math.min(c,Math.max(n,i)).toFixed(p);this.setAttribute("value",h),this.emit("ui-change",{value:Number(h)})};(r=this.$(".dec"))==null||r.addEventListener("click",()=>t(((e==null?void 0:e.valueAsNumber)||0)-this.getNumAttr("step",1))),(o=this.$(".inc"))==null||o.addEventListener("click",()=>t(((e==null?void 0:e.valueAsNumber)||0)+this.getNumAttr("step",1))),e==null||e.addEventListener("change",()=>t(e.valueAsNumber))}}a(M,"observedAttributes",["label","value","min","max","step","precision","disabled","readonly","helper-text"]);l("ui-number-input",M);class P extends d{styles(){return":host{display:inline-grid;gap:.25rem}.stars{display:flex;gap:.15rem}button{all:unset;cursor:pointer;font-size:1.4rem;line-height:1;color:var(--ui-color-border,#cbd5e1)}button.on{color:var(--ui-color-warning,#f59e0b)}button:focus-visible{outline:2px solid var(--ui-color-primary,#4f46e5);outline-offset:2px}button:disabled{cursor:default}small{color:var(--ui-color-text-secondary,#64748b)}"}template(){const e=this.getNumAttr("value",0),t=this.getNumAttr("max",5);return`<span class="stars" role="radiogroup" aria-label="${this.getAttr("label","Rating")}">${Array.from({length:t},(r,o)=>`<button type="button" class="${o<e?"on":""}" role="radio" aria-checked="${o+1===e}" data-value="${o+1}" ${this.getBoolAttr("readonly")||this.getBoolAttr("disabled")?"disabled":""}>★</button>`).join("")}</span><small>${e} of ${t}</small>`}onRendered(){this.$$("button").forEach(e=>e.addEventListener("click",()=>{const t=Number(e.dataset.value);this.setAttribute("value",String(t)),this.emit("ui-change",{value:t})}))}}a(P,"observedAttributes",["value","max","label","readonly","disabled"]);l("ui-rating",P);class F extends d{styles(){return":host{display:inline-flex}:host([full-width]){display:flex;width:100%}.group{display:flex;flex-direction:var(--_direction,row);width:100%}:host([orientation=vertical]){--_direction:column}:host([attached]) ::slotted(ui-button:not(:first-child)),:host([attached]) ::slotted(ui-icon-button:not(:first-child)){margin-left:-1px}"}template(){return'<div class="group" role="group"><slot></slot></div>'}}a(F,"observedAttributes",["orientation","attached","full-width"]);l("ui-button-group",F);class H extends d{constructor(){super(...arguments);a(this,"_tags",[])}set tags(t){this._tags=Array.isArray(t)?t:[],this.render()}get tags(){return this._tags}styles(){return":host{display:grid;gap:.35rem}label{font-size:.875rem;font-weight:600}.control{display:flex;align-items:center;flex-wrap:wrap;gap:.35rem;min-height:2.6rem;padding:.35rem;border:1px solid var(--ui-color-border,#cbd5e1);border-radius:.5rem;background:var(--ui-color-surface)}.tag{display:inline-flex;align-items:center;gap:.25rem;padding:.2rem .45rem;border-radius:999px;background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 14%,transparent);font-size:.8rem}.tag button{all:unset;cursor:pointer}input{flex:1;min-width:8rem;padding:.25rem;border:0;outline:0;background:transparent;color:var(--ui-color-text);font:inherit}"}template(){return`<label>${this.getAttr("label")}</label><div class="control">${this.tags.map((t,r)=>`<span class="tag">${t}<button type="button" data-index="${r}" aria-label="Remove ${t}">×</button></span>`).join("")}<input placeholder="${this.getAttr("placeholder")}" ${this.getBoolAttr("disabled")?"disabled":""}/></div>`}onRendered(){const t=this.$("input");t==null||t.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===",")&&t.value.trim()&&(r.preventDefault(),this.addTag(t.value.trim())),r.key==="Backspace"&&!t.value&&this.tags.length&&this.removeTag(this.tags.length-1)}),this.$$("[data-index]").forEach(r=>r.addEventListener("click",()=>this.removeTag(Number(r.dataset.index))))}addTag(t){this.tags.length>=this.getNumAttr("max-tags",1/0)||!this.getBoolAttr("allow-duplicates")&&this.tags.includes(t)||(this._tags=[...this.tags,t],this.render(),this.emit("ui-change",{tags:this.tags}))}removeTag(t){this._tags=this.tags.filter((r,o)=>o!==t),this.render(),this.emit("ui-change",{tags:this.tags})}}a(H,"observedAttributes",["label","placeholder","disabled","max-tags","allow-duplicates"]);l("ui-tag-input",H);class W extends d{styles(){return":host{display:grid;gap:.4rem}label{font-size:.875rem;font-weight:600}.cells{display:flex;gap:.45rem}input{width:2.5rem;height:2.8rem;text-align:center;border:1px solid var(--ui-color-border,#cbd5e1);border-radius:.45rem;background:var(--ui-color-surface);color:var(--ui-color-text);font:600 1.1rem var(--ui-font-mono,monospace)}input:focus{outline:2px solid color-mix(in srgb,var(--ui-color-primary,#4f46e5) 28%,transparent);border-color:var(--ui-color-primary,#4f46e5)}"}template(){const e=this.getNumAttr("length",6),t=this.getAttr("value");return`<label>${this.getAttr("label")}</label><div class="cells">${Array.from({length:e},(r,o)=>`<input aria-label="Character ${o+1} of ${e}" maxlength="1" value="${t[o]??""}" inputmode="${this.getBoolAttr("numeric")?"numeric":"text"}" type="${this.getBoolAttr("masked")?"password":"text"}" ${this.getBoolAttr("disabled")?"disabled":""}/>`).join("")}</div>`}onRendered(){const e=this.$$("input"),t=()=>{const r=e.map(o=>o.value).join("");this.setAttribute("value",r),this.emit("ui-change",{value:r,complete:r.length===e.length})};e.forEach((r,o)=>{r.addEventListener("input",()=>{var i;this.getBoolAttr("numeric")&&(r.value=r.value.replace(/\D/g,"")),r.value&&((i=e[o+1])==null||i.focus()),t()}),r.addEventListener("keydown",i=>{var n;i.key==="Backspace"&&!r.value&&((n=e[o-1])==null||n.focus())}),r.addEventListener("paste",i=>{var c,p;i.preventDefault();const n=((c=i.clipboardData)==null?void 0:c.getData("text").slice(0,e.length))??"";[...n].forEach((u,h)=>{e[h]&&(e[h].value=u)}),t(),(p=e[Math.min(n.length,e.length)-1])==null||p.focus()})})}}a(W,"observedAttributes",["label","length","value","numeric","disabled","masked"]);l("ui-otp-input",W);class X extends d{constructor(){super(...arguments);a(this,"dragging",!1)}styles(){return":host{display:block}.zone{display:grid;place-items:center;gap:.4rem;min-height:9rem;padding:1.25rem;text-align:center;border:1.5px dashed var(--ui-color-border,#cbd5e1);border-radius:.75rem;background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 3%,transparent);cursor:pointer}.zone.dragging{border-color:var(--ui-color-primary,#4f46e5);background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 10%,transparent)}strong{font-size:.95rem}small{color:var(--ui-color-text-secondary,#64748b)}input{position:absolute;inline-size:1px;block-size:1px;opacity:0}"}template(){return`<label class="zone ${this.dragging?"dragging":""}" tabindex="0"><strong>${this.getAttr("label","Drop files or browse")}</strong><small>${this.getAttr("description")}</small><input type="file" accept="${this.getAttr("accept")}" ${this.getBoolAttr("multiple")?"multiple":""} ${this.getBoolAttr("disabled")?"disabled":""}/></label>`}onRendered(){const t=this.$(".zone"),r=this.$("input"),o=i=>{const n=this.getNumAttr("max-size",1/0),c=i.filter(p=>p.size<=n);this.emit("ui-files",{files:c,rejected:i.filter(p=>!c.includes(p))})};r==null||r.addEventListener("change",()=>o(Array.from(r.files??[]))),t==null||t.addEventListener("dragover",i=>{i.preventDefault(),this.dragging=!0,t.classList.add("dragging")}),t==null||t.addEventListener("dragleave",()=>{this.dragging=!1,t.classList.remove("dragging")}),t==null||t.addEventListener("drop",i=>{var n;i.preventDefault(),this.dragging=!1,t.classList.remove("dragging"),o(Array.from(((n=i.dataTransfer)==null?void 0:n.files)??[]))}),t==null||t.addEventListener("keydown",i=>{(i.key==="Enter"||i.key===" ")&&(i.preventDefault(),r==null||r.click())})}}a(X,"observedAttributes",["label","description","accept","multiple","disabled","max-size"]);l("ui-drop-zone",X);class A extends d{styles(){return":host{display:grid;gap:.35rem}label{font-size:.875rem;font-weight:600}input{padding:.58rem .7rem;border:1px solid var(--ui-color-border,#cbd5e1);border-radius:.5rem;background:var(--ui-color-surface);color:var(--ui-color-text);font:inherit;color-scheme:dark}input:focus{outline:2px solid color-mix(in srgb,var(--ui-color-primary,#4f46e5) 28%,transparent);border-color:var(--ui-color-primary,#4f46e5)}small{color:var(--ui-color-text-secondary,#64748b)}"}template(){return`<label>${this.getAttr("label")}</label><input type="${this.inputType}" value="${this.getAttr("value")}" min="${this.getAttr("min")}" max="${this.getAttr("max")}" step="${this.getAttr("step")}" ${this.getBoolAttr("disabled")?"disabled":""} ${this.getBoolAttr("readonly")?"readonly":""}/><small>${this.getAttr("helper-text")}</small>`}onRendered(){const e=this.$("input");e==null||e.addEventListener("change",()=>{this.setAttribute("value",e.value),this.emit("ui-change",{value:e.value})})}}a(A,"observedAttributes",["label","value","min","max","step","disabled","readonly","helper-text"]);class ee extends A{constructor(){super(...arguments);a(this,"inputType","date")}}class re extends A{constructor(){super(...arguments);a(this,"inputType","time")}}l("ui-date-picker",ee);l("ui-time-picker",re);class V extends d{styles(){return`
      :host { display: inline-flex; }
      :host([disabled]) { opacity: 0.5; pointer-events: none; }
    ...
    `}onRendered(){var e;(e=this.$(".delete-btn"))==null||e.addEventListener("click",t=>{t.stopPropagation(),this.emit("ui-delete")})}}a(V,"observedAttributes",["variant","size","color","deletable","disabled"]);l("ui-chip",V);class O extends d{styles(){return`
    ...
    `}template(){const e=this.getAttr("variant","standard"),t=this.getAttr("content"),r=this.getNumAttr("max",99),o=e==="dot";let i=t;if(!o&&t){const n=Number(t);!isNaN(n)&&n>r&&(i=`${r}+`)}return`
      <slot></slot>
      <span class="badge ${o?"dot":""}" part="badge" aria-label="${t||""}">${o?"":i}</span>
    `}}a(O,"observedAttributes",["content","variant","color","max"]);l("ui-badge",O);class U extends d{styles(){return`
      :host { display: inline-flex; }
    ...
    `}template(){const e=this.getAttr("src"),t=this.getAttr("alt",""),r=this.getAttr("size","md"),o=this.getAttr("variant","circular"),i=this.getAttr("initials"),n=e?`<img src="${e}" alt="${t}" />`:`<span class="fallback">${i||"?"}</span>`;return`<span class="avatar size-${r} variant-${o}" part="avatar" role="img" aria-label="${t||i||"avatar"}">${n}</span>`}}a(U,"observedAttributes",["src","alt","size","variant","initials"]);l("ui-avatar",U);class Y extends d{constructor(){super(...arguments);a(this,"_timer",null);a(this,"_show",()=>{const t=this.getNumAttr("delay",200);this._timer=setTimeout(()=>this.classList.add("show"),t)});a(this,"_hide",()=>{this._timer&&clearTimeout(this._timer),this.classList.remove("show")})}styles(){return`
    ...
    `}onConnected(){this.addEventListener("mouseenter",this._show),this.addEventListener("mouseleave",this._hide),this.addEventListener("focusin",this._show),this.addEventListener("focusout",this._hide)}onDisconnected(){this.removeEventListener("mouseenter",this._show),this.removeEventListener("mouseleave",this._hide),this.removeEventListener("focusin",this._show),this.removeEventListener("focusout",this._hide),this._timer&&clearTimeout(this._timer)}}a(Y,"observedAttributes",["content","placement","delay"]);l("ui-tooltip",Y);class K extends d{styles(){return`
      :host { display: block; }
    ...
    `}template(){const t=this.getBoolAttr("sortable"),r=this.getNumAttr("page-size",0);if(this._columns.length===0)return'<div class="empty" part="empty">No columns configured</div>';let o=this._rows.map((u,h)=>({row:u,index:h,rowId:this.resolveRowId(u,h)}));t&&this._sortField&&o.sort((u,h)=>{const b=String(u.row[this._sortField]??""),g=String(h.row[this._sortField]??""),y=b.localeCompare(g,void 0,{numeric:!0});return this._sortDir==="asc"?y:-y});const i=r>0?Math.ceil(o.length/r):1;r>0&&(o=o.slice(this._page*r,(this._page+1)*r));const n=this._columns.map(u=>{const b=this._sortField===u.field?this._sortDir==="asc"?"▲":"▼":"",g=u.width?` style="width: ${u.width}px"`:"",y=u.align?` style="text-align: ${u.align}"`:"";return`<th class="${t&&u.sortable!==!1?"sortable":""}" data-field="${u.field}"${g}${y}>${u.headerName}${b?`<span class="sort-icon">${b}</span>`:""}</th>`}).join(""),c=o.length===0?`<tr><td colspan="${this._columns.length}" class="empty">No data</td></tr>`:o.map(u=>`<tr data-row-index="${u.index}" class="${u.rowId===this._selectedRowId?"row-selected":""}">`+this._columns.map(h=>{const b=u.row[h.field]??"";return`<td${h.align?` style="text-align:${h.align}"`:""}>${b}</td>`}).join("")+"</tr>").join(""),p=r>0&&i>1?`<div class="pagination" part="pagination">
      ...
      ${p}
    `}onRendered(){var t,r;this.$$("th.sortable").forEach(o=>{o.addEventListener("click",()=>{const i=o.dataset.field;this._sortField===i?this._sortDir=this._sortDir==="asc"?"desc":"asc":(this._sortField=i,this._sortDir="asc"),this.render(),this.emit("ui-sort",{field:this._sortField,direction:this._sortDir})})}),(t=this.$('[data-action="prev"]'))==null||t.addEventListener("click",()=>{this._page>0&&(this._page--,this.render(),this.emit("ui-page",{page:this._page}))}),(r=this.$('[data-action="next"]'))==null||r.addEventListener("click",()=>{const o=this.getNumAttr("page-size",0),i=o>0?Math.ceil(this._rows.length/o):1;this._page<i-1&&(this._page++,this.render(),this.emit("ui-page",{page:this._page}))}),this.$$("tbody tr[data-row-index]").forEach(o=>{o.addEventListener("click",()=>{const i=Number(o.dataset.rowIndex??"-1");if(i<0)return;const n=this._rows[i];if(!n)return;const c=this.resolveRowId(n,i);this._selectedRowId=c,this.render(),this.emit("ui-row-click",{rowId:c,row:n})})})}}a(Q,"observedAttributes",["striped","bordered","sortable","page-size"]);l("ui-data-grid",Q);class J extends d{styles(){return":host{display:block}.item{display:flex;align-items:center;gap:.75rem;min-height:3rem;padding:.55rem .75rem;border-radius:var(--ui-radius-md,.5rem)}:host([interactive]) .item{cursor:pointer}:host([interactive]) .item:hover{background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 8%,transparent)}:host([selected]) .item{background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 14%,transparent);color:var(--ui-color-primary,#4f46e5)}:host([disabled]){opacity:.45;pointer-events:none}::slotted([slot=secondary]){margin-inline-start:auto;color:var(--ui-color-text-secondary,#64748b)}"}template(){return`<div class="item" role="listitem" tabindex="${this.getBoolAttr("interactive")?"0":"-1"}"><slot name="leading"></slot><slot></slot><slot name="secondary"></slot></div>`}onRendered(){var e;this.getBoolAttr("interactive")&&((e=this.$(".item"))==null||e.addEventListener("click",()=>this.emit("ui-select",{value:this.getAttr("value")})))}}a(J,"observedAttributes",["selected","disabled","interactive","value"]);class G extends d{styles(){return":host{display:grid}:host([dividers]) ::slotted(ui-list-item:not(:last-child)){border-bottom:1px solid var(--ui-color-border,#cbd5e1)}:host([dense]) ::slotted(ui-list-item){--ui-list-min-height:2.25rem}"}template(){return`<div role="list" aria-label="${this.getAttr("label","List")}"><slot></slot></div>`}}a(G,"observedAttributes",["dense","dividers","label"]);l("ui-list-item",J);l("ui-list",G);class Z extends d{constructor(){super(...arguments);a(this,"_columns",[]);a(this,"_rows",[]);a(this,"sortKey","");a(this,"sortDirection","asc")}set columns(t){this._columns=t??[],this.render()}get columns(){return this._columns}set rows(t){this._rows=t??[],this.render()}get rows(){return this._rows}styles(){return":host{display:block;overflow:auto;width:100%}table{width:100%;border-collapse:collapse;font-size:.875rem}caption{text-align:left;padding:.5rem 0;font-weight:700}th,td{padding:var(--_pad,.65rem .75rem);border-bottom:1px solid var(--ui-color-border,#cbd5e1);text-align:left}th{color:var(--ui-color-text-secondary,#64748b);font-size:.75rem;text-transform:uppercase;letter-spacing:.04em}th button{all:unset;cursor:pointer;display:inline-flex;gap:.35rem;align-items:center}:host([dense]){--_pad:.4rem .55rem}:host([striped]) tbody tr:nth-child(even){background:color-mix(in srgb,var(--ui-color-text,#fff) 3%,transparent)}:host([hover]) tbody tr:hover{background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 8%,transparent)}.empty{text-align:center;color:var(--ui-color-text-secondary,#64748b)}"}template(){const t=[...this.rows].sort((r,o)=>this.sortKey?String(r[this.sortKey]??"").localeCompare(String(o[this.sortKey]??""))*(this.sortDirection==="asc"?1:-1):0);return`<table><caption>${this.getAttr("caption")}</caption><thead><tr>${this.columns.map(r=>`<th style="text-align:${r.align??"left"}">${r.sortable?`<button data-key="${r.key}">${r.label}<span>${this.sortKey===r.key?this.sortDirection==="asc"?"↑":"↓":"↕"}</span></button>`:r.label}</th>`).join("")}</tr></thead><tbody>${t.length?t.map((r,o)=>`<tr data-index="${o}">${this.columns.map(i=>`<td style="text-align:${i.align??"left"}">${String(r[i.key]??"")}</td>`).join("")}</tr>`).join(""):`<tr><td class="empty" colspan="${Math.max(1,this.columns.length)}">${this.getAttr("empty-message","No rows")}</td></tr>`}</tbody></table>`}onRendered(){this.$$("th button").forEach(t=>t.addEventListener("click",()=>{const r=t.dataset.key??"";this.sortDirection=this.sortKey===r&&this.sortDirection==="asc"?"desc":"asc",this.sortKey=r,this.render(),this.emit("ui-sort",{key:r,direction:this.sortDirection})})),this.$$("tbody tr[data-index]").forEach(t=>t.addEventListener("click",()=>this.emit("ui-row-select",{index:Number(t.dataset.index)})))}}a(Z,"observedAttributes",["striped","hover","dense","caption","empty-message"]);l("ui-table",Z);class tt extends d{styles(){return':host{display:grid;grid-template-columns:5.5rem 1rem 1fr;gap:.65rem;min-height:3.5rem;position:relative}.time{color:var(--ui-color-text-secondary,#64748b);font-size:.75rem;text-align:right;padding-top:.15rem}.rail{position:relative}.rail:before{content:"";position:absolute;left:50%;top:1rem;bottom:-.5rem;border-left:1px solid var(--ui-color-border,#cbd5e1)}.dot{display:block;width:.7rem;height:.7rem;margin:.15rem auto 0;border-radius:50%;background:var(--ui-color-primary,#4f46e5);box-shadow:0 0 0 3px color-mix(in srgb,var(--ui-color-primary,#4f46e5) 18%,transparent)}:host([status=warning]) .dot{background:var(--ui-color-warning,#f59e0b)}:host([status=error]) .dot{background:var(--ui-color-error,#ef4444)}.content{display:grid;gap:.15rem}.content small{color:var(--ui-color-text-secondary,#64748b)}'}template(){return`<time class="time">${this.getAttr("time")}</time><span class="rail"><span class="dot"></span></span><span class="content"><strong>${this.getAttr("title")}</strong><small><slot></slot></small></span>`}}a(tt,"observedAttributes",["time","title","status"]);class ie extends d{styles(){return":host{display:grid}"}template(){return'<div role="list"><slot></slot></div>'}}l("ui-timeline-item",tt);l("ui-timeline",ie);class et extends d{styles(){return":host{display:block}.row{display:flex;align-items:center;gap:.35rem;min-height:2rem;padding:.25rem .4rem;border-radius:.3rem;cursor:pointer}:host([selected]) .row{color:var(--ui-color-primary,#4f46e5);background:color-mix(in srgb,var(--ui-color-primary,#4f46e5) 12%,transparent)}:host([disabled]){opacity:.45;pointer-events:none}.toggle{all:unset;width:1rem;text-align:center}.children{margin-left:1.2rem}:host(:not([expanded])) .children{display:none}"}template(){const e=this.children.length>0;return`<div class="row" role="treeitem" tabindex="0" aria-expanded="${e?this.getBoolAttr("expanded"):""}" aria-selected="${this.getBoolAttr("selected")}"><button class="toggle" type="button" aria-label="Toggle children">${e?this.getBoolAttr("expanded")?"▾":"▸":"·"}</button><span>${this.getAttr("label")}<slot name="label"></slot></span></div><div class="children" role="group"><slot></slot></div>`}onRendered(){var t,r;const e=()=>{this.toggleAttribute("selected",!0),this.emit("ui-select",{value:this.getAttr("value")})};(t=this.$(".row"))==null||t.addEventListener("click",e),(r=this.$(".toggle"))==null||r.addEventListener("click",o=>{o.stopPropagation(),this.toggleAttribute("expanded"),this.emit("ui-toggle",{value:this.getAttr("value"),expanded:this.getBoolAttr("expanded")})})}}a(et,"observedAttributes",["label","expanded","selected","disabled","value"]);class rt extends d{template(){return`<div role="tree" aria-label="${this.getAttr("label","Tree")}"><slot></slot></div>`}}a(rt,"observedAttributes",["label"]);l("ui-tree-item",et);l("ui-tree-view",rt);class it extends d{styles(){return":host{display:block}.text{margin:0;color:var(--_color,var(--ui-color-text));font-size:var(--_size,1rem);line-height:var(--_line,1.5);font-weight:var(--_weight,400);text-align:var(--_align,start)}:host([color=muted]){--_color:var(--ui-color-text-secondary,#64748b)}:host([variant=h1]){--_size:2.25rem;--_line:1.15;--_weight:700}:host([variant=h2]){--_size:1.75rem;--_line:1.2;--_weight:700}:host([variant=h3]){--_size:1.35rem;--_line:1.25;--_weight:650}:host([variant=subtitle]){--_size:1.05rem;--_weight:600}:host([variant=caption]){--_size:.75rem;--_line:1.35}:host([variant=overline]){--_size:.7rem;--_weight:700;text-transform:uppercase;letter-spacing:.1em}:host([truncate]) .text{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}:host([lines]) .text{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:var(--_lines);overflow:hidden}"}template(){const e=this.getAttr("variant","body"),t=e==="h1"||e==="h2"||e==="h3"?e:"p";return`<${t} class="text" style="--_align:${this.getAttr("align","start")};--_weight:${this.getAttr("weight","")};--_lines:${this.getNumAttr("lines",2)}"><slot></slot></${t}>`}}a(it,"observedAttributes",["variant","align","color","weight","truncate","lines"]);l("ui-typography",it);class st extends d{styles(){return":host{display:inline-flex}.group{display:flex;align-items:center}::slotted(ui-avatar){margin-left:var(--_spacing,-.55rem);box-shadow:0 0 0 2px var(--ui-color-surface)}::slotted(ui-avatar:first-child){margin-left:0}"}template(){return`<div class="group" role="group" aria-label="${this.getAttr("label","People")}" style="--_spacing:${this.getAttr("spacing","-.55rem")}"><slot></slot></div>`}}a(st,"observedAttributes",["spacing","max","label"]);l("ui-avatar-group",st);class ot extends d{styles(){return":host{display:block}.callout{display:grid;grid-template-columns:auto 1fr;gap:.7rem;padding:var(--_pad,.9rem 1rem);border-left:4px solid var(--_tone,var(--ui-color-primary,#4f46e5));background:color-mix(in srgb,var(--_tone,var(--ui-color-primary,#4f46e5)) 9%,transparent);border-radius:.35rem}:host([compact]){--_pad:.55rem .7rem}:host([tone=success]){--_tone:var(--ui-color-success,#22c55e)}:host([tone=warning]){--_tone:var(--ui-color-warning,#f59e0b)}:host([tone=error]){--_tone:var(--ui-color-error,#ef4444)}.copy{display:grid;gap:.25rem}.copy small{color:var(--ui-color-text-secondary,#64748b)}"}template(){return`<span><slot name="icon">◇</slot></span><span class="copy"><strong>${this.getAttr("title")}</strong><small><slot></slot></small></span>`}}a(ot,"observedAttributes",["title","tone","compact"]);l("ui-callout",ot);class at extends d{styles(){return":host{display:block}.list{display:grid;grid-template-columns:repeat(var(--_columns,3),minmax(0,1fr));gap:var(--_gap,.5rem);grid-auto-rows:var(--_row,8rem)}:host([variant=masonry]) .list{display:block;columns:var(--_columns,3);column-gap:var(--_gap,.5rem)}:host([variant=masonry]) ::slotted(*){display:block;break-inside:avoid;margin-bottom:var(--_gap,.5rem)}::slotted(*){overflow:hidden;border-radius:.4rem}"}template(){return`<div class="list" style="--_columns:${this.getNumAttr("columns",3)};--_gap:${this.getAttr("gap",".5rem")};--_row:${this.getAttr("row-height","8rem")}"><slot></slot></div>`}}a(at,"observedAttributes",["columns","gap","row-height","variant"]);l("ui-image-list",at);class nt extends d{constructor(){super(...arguments);a(this,"_items",[]);a(this,"scrollOffset",0)}set items(t){this._items=Array.isArray(t)?t:[],this.render()}get items(){return this._items}styles(){return":host{display:block}.viewport{height:var(--_height,18rem);overflow:auto;border:1px solid var(--ui-color-border,#cbd5e1);border-radius:.5rem}.spacer{position:relative}.item{position:absolute;left:0;right:0;height:var(--_item,2.5rem);display:flex;align-items:center;padding:0 .75rem;border-bottom:1px solid var(--ui-color-border,#cbd5e1);font-size:.875rem}"}template(){const t=this.getNumAttr("item-height",40),r=this.getNumAttr("height",288),o=Math.max(0,Math.floor(this.scrollOffset/t)-2),i=Math.ceil(r/t)+4,n=Math.min(this.items.length,o+i),c=(p,u)=>String(typeof p=="object"&&p!==null?p.label??p.name??JSON.stringify(p):p??u);return`<div class="viewport" role="list" aria-label="${this.getAttr("label","Virtual list")}" style="--_height:${r}px;--_item:${t}px"><div class="spacer" style="height:${this.items.length*t}px">${this.items.slice(o,n).map((p,u)=>`<div class="item" role="listitem" style="top:${(o+u)*t}px">${c(p,o+u)}</div>`).join("")}</div></div>`}onRendered(){const t=this.$(".viewport");t&&(t.scrollTop=this.scrollOffset),t==null||t.addEventListener("scroll",()=>{this.scrollOffset=t.scrollTop,this.render()})}}a(nt,"observedAttributes",["height","item-height","label"]);l("ui-virtual-list",nt);class lt extends d{styles(){return`
      :host { display: block; }
      :host([hidden]) { display: none !important; }
    ...
    `}template(){const e=this.getAttr("variant","linear"),t=this.getAttr("size","md"),r=this.hasAttribute("value"),o=this.getNumAttr("value",0);if(e==="circular"){const i=2*Math.PI*18,n=r?i*(1-o/100):i*.75;return`
    ...
    `}}a(ct,"observedAttributes",["value","variant","size","color"]);l("ui-progress",ct);class ut extends d{styles(){return`
      :host { display: block; }
    ...
    `}template(){const e=this.getAttr("variant","text"),t=this.getAttr("width",e==="circular"?"40px":"100%"),r=this.getAttr("height",e==="circular"?"40px":e==="text"?"1em":"100px"),o=this.getAttr("animation","pulse");return`<span
      ...
      aria-hidden="true"
      part="skeleton"
    ></span>`}}a(ut,"observedAttributes",["variant","width","height","animation"]);l("ui-skeleton",ut);class se extends HTMLElement{constructor(){super();a(this,"root");this.root=this.attachShadow({mode:"open"}),this.root.innerHTML=`
      <style>
        :host {
          position: fixed;
          z-index: 99999;
    ...
    `,this.root.appendChild(i);const n=()=>{i.classList.add("removing"),setTimeout(()=>i.remove(),200)};(c=i.querySelector(".close"))==null||c.addEventListener("click",n),o>0&&setTimeout(n,o)}}l("ui-toast-container",se);class pt extends d{styles(){return":host{display:none;position:fixed;inset:0;z-index:100}:host([open]){display:grid;place-items:center}.backdrop{position:absolute;inset:0;background:rgb(2 6 23/.62);backdrop-filter:blur(var(--_blur,0))}:host([blur]){--_blur:4px}:host([invisible]) .backdrop{background:transparent}.content{position:relative;z-index:1}"}template(){return'<div class="backdrop" part="backdrop"></div><div class="content"><slot></slot></div>'}onRendered(){var e;(e=this.$(".backdrop"))==null||e.addEventListener("click",()=>this.emit("ui-dismiss"))}}a(pt,"observedAttributes",["open","blur","invisible"]);l("ui-backdrop",pt);class ht extends d{styles(){return":host{display:grid;place-items:center;text-align:center;padding:var(--_padding,2.5rem);border:1px dashed var(--ui-color-border,#cbd5e1);border-radius:var(--ui-radius-lg,.75rem)}:host([compact]){--_padding:1rem}.wrap{display:grid;gap:.55rem;justify-items:center;max-width:30rem}.icon{font-size:1.75rem;color:var(--ui-color-text-secondary,#64748b)}h3,p{margin:0}p{color:var(--ui-color-text-secondary,#64748b);font-size:.875rem}.actions{margin-top:.5rem}"}template(){return`<div class="wrap"><div class="icon"><slot name="icon">◇</slot></div><h3>${this.getAttr("title")}</h3><p>${this.getAttr("description")}</p><div class="actions"><slot></slot></div></div>`}}a(ht,"observedAttributes",["title","description","compact"]);l("ui-empty-state",ht);class bt extends d{constructor(){super(...arguments);a(this,"timer")}styles(){return":host{display:none;position:fixed;z-index:110;inset:auto 1rem 1rem auto}:host([open]){display:block}:host([position=bottom-left]){inset:auto auto 1rem 1rem}:host([position=top-right]){inset:1rem 1rem auto auto}:host([position=top-left]){inset:1rem auto auto 1rem}.bar{display:flex;align-items:center;gap:.75rem;min-width:18rem;max-width:32rem;padding:.75rem 1rem;border-radius:var(--ui-radius-md,.5rem);background:var(--ui-color-surface);color:var(--ui-color-text);border:1px solid var(--ui-color-border,#cbd5e1);box-shadow:0 14px 34px rgb(0 0 0/.28)}:host([severity=success]) .bar{border-left:4px solid var(--ui-color-success,#22c55e)}:host([severity=warning]) .bar{border-left:4px solid var(--ui-color-warning,#f59e0b)}:host([severity=error]) .bar{border-left:4px solid var(--ui-color-error,#ef4444)}button{all:unset;margin-left:auto;cursor:pointer;font-size:1.1rem}"}template(){return`<div class="bar" role="status" aria-live="polite"><span>${this.getAttr("message")}<slot></slot></span>${this.getBoolAttr("closable")?'<button type="button" aria-label="Dismiss">×</button>':""}</div>`}onRendered(){var t;window.clearTimeout(this.timer),(t=this.$("button"))==null||t.addEventListener("click",()=>this.close("dismiss")),this.getBoolAttr("open")&&this.getNumAttr("duration",0)>0&&(this.timer=window.setTimeout(()=>this.close("timeout"),this.getNumAttr("duration")))}onDisconnected(){window.clearTimeout(this.timer)}close(t){this.removeAttribute("open"),this.emit("ui-close",{reason:t})}}a(bt,"observedAttributes",["open","message","severity","position","duration","closable"]);l("ui-snackbar",bt);class mt extends d{get value(){return this.getAttr("value")}set value(e){this.setAttribute("value",e)}styles(){return`
      :host { display: block; }
      .tablist {
        display: flex;
        gap: 0;
    ...
    `}onRendered(){var e;(e=this.$("button"))==null||e.addEventListener("click",()=>{this.getBoolAttr("disabled")||this.dispatchEvent(new CustomEvent("tab-select",{bubbles:!0,composed:!0,detail:{value:this.getAttr("value")}}))})}}a(gt,"observedAttributes",["value","label","active","disabled"]);class vt extends d{styles(){return`
      :host { display: none; }
      :host([active]) { display: block; padding: var(--ui-space-4, 1rem) 0; }
    `}template(){return"<slot></slot>"}}a(vt,"observedAttributes",["value","active"]);l("ui-tabs",mt);l("ui-tab",gt);l("ui-tab-panel",vt);class ft extends d{get open(){return this.getBoolAttr("open")}set open(e){e?this.setAttribute("open",""):this.removeAttribute("open")}styles(){return`
      :host { display: contents; }
    ...
    `}onRendered(){var e;(e=this.$(".backdrop"))==null||e.addEventListener("click",()=>{this.open=!1,this.emit("ui-close")})}}a(ft,"observedAttributes",["open","anchor","size","overlay"]);l("ui-drawer",ft);class xt extends d{styles(){return`
      :host { display: block; }
    ...
    `}onConnected(){requestAnimationFrame(()=>this._insertSeparators())}_insertSeparators(){const e=this.getAttr("separator","/"),t=Array.from(this.children).filter(r=>!r.classList.contains("_bc-sep"));this.querySelectorAll("._bc-sep").forEach(r=>r.remove()),t.forEach((r,o)=>{if(o<t.length-1){const i=document.createElement("span");i.className="_bc-sep",i.setAttribute("aria-hidden","true"),i.textContent=` ${e} `,i.style.color="var(--ui-color-text-secondary, #64748b)",i.style.fontSize="var(--ui-text-sm, 0.875rem)",r.after(i)}})}}a(xt,"observedAttributes",["separator"]);l("ui-breadcrumbs",xt);class yt extends d{get page(){return this.getNumAttr("page",1)}set page(e){this.setAttribute("page",String(e))}styles(){return`
      :host { display: inline-flex; }
    ...
    `}template(){const e=this.getNumAttr("count",1),t=this.getNumAttr("page",1),r=this.getNumAttr("sibling-count",1),i=this._buildRange(e,t,r).map(n=>{if(n==="...")return'<span class="ellipsis">…</span>';const c=Number(n);return`<button data-page="${c}" class="${c===t?"active":""}" aria-current="${c===t?"page":"false"}">${c}</button>`}).join("");return`
    ...
    `}template(){return'<div class="accordion" part="accordion"><slot></slot></div>'}onConnected(){this.addEventListener("accordion-toggle",(e=>{this.getBoolAttr("multiple")||this.querySelectorAll("ui-accordion-item").forEach(r=>{r!==e.detail.element&&r.removeAttribute("open")}),this.emit("ui-change",{value:e.detail.value,open:e.detail.open})}))}}a(Lt,"observedAttributes",["multiple"]);class Nt extends d{get open(){return this.getBoolAttr("open")}set open(e){e?this.setAttribute("open",""):this.removeAttribute("open")}styles(){return`
      :host { display: block; }
    ...
    `}onRendered(){var e;(e=this.$(".header"))==null||e.addEventListener("click",()=>{this.getBoolAttr("disabled")||(this.open=!this.open,this.render(),this.dispatchEvent(new CustomEvent("accordion-toggle",{bubbles:!0,composed:!0,detail:{value:this.getAttr("value"),open:this.open,element:this}})))})}}a(Nt,"observedAttributes",["value","label","open","disabled"]);l("ui-accordion",Lt);l("ui-accordion-item",Nt);class Bt extends d{styles(){return`
      :host { display: block; }
    ...
    `}}a(Bt,"observedAttributes",["variant","elevation"]);l("ui-paper",Bt);class It extends d{styles(){return":host { display: block; }"}template(){const e=this.getAttr("p"),t=this.getAttr("m"),r=this.getAttr("display"),o=this.getAttr("bg"),i=[e?`padding:${e}`:"",t?`margin:${t}`:"",r?`display:${r}`:"",o?`background:${o}`:""].filter(Boolean).join(";");return`<div ${i?`style="${i}"`:""} part="box"><slot></slot></div>`}}a(It,"observedAttributes",["p","m","display","bg"]);l("ui-box",It);class Ct extends d{styles(){return`
      :host { display: flex; }
    `}template(){const e=this.getAttr("direction","column"),t=this.getAttr("gap","var(--ui-space-3, 0.75rem)"),r=this.getAttr("align"),o=this.getAttr("justify"),i=this.getBoolAttr("wrap");return`<div style="${["display:flex",`flex-direction:${e}`,`gap:${t}`,r?`align-items:${r}`:"",o?`justify-content:${o}`:"",i?"flex-wrap:wrap":""].filter(Boolean).join(";")}" part="stack"><slot></slot></div>`}}a(Ct,"observedAttributes",["direction","gap","align","justify","wrap"]);l("ui-stack",Ct);class Tt extends d{styles(){return":host { display: block; }"}template(){const e=this.getAttr("columns","repeat(12, 1fr)"),t=this.getAttr("gap","var(--ui-space-4, 1rem)"),r=this.getAttr("rows");return`<div style="${["display:grid",`grid-template-columns:${e}`,`gap:${t}`,r?`grid-template-rows:${r}`:""].filter(Boolean).join(";")}" part="grid"><slot></slot></div>`}}a(Tt,"observedAttributes",["columns","gap","rows"]);l("ui-grid",Tt);class Rt extends d{styles(){return`
      :host { display: block; }
      .divider {
        border: none;
        background: var(--ui-color-border, #e2e8f0);
    ...
    `}template(){const e=this.getAttr("orientation","horizontal"),t=this.getAttr("color");return`<hr class="divider ${e}" role="separator" aria-orientation="${e}" ${t?`style="background:${t}"`:""} part="divider" />`}}a(Rt,"observedAttributes",["orientation","color"]);l("ui-divider",Rt);class jt extends d{styles(){return":host{display:block;width:100%}.container{width:100%;max-width:var(--_max,72rem);margin-inline:auto;padding-inline:var(--_pad,1rem)}"}template(){const e=this.getAttr("max-width","lg"),t={xs:"28rem",sm:"40rem",md:"52rem",lg:"64rem",xl:"80rem"};return`<div class="container" style="--_max:${this.getBoolAttr("fluid")?"none":t[e]??e};--_pad:${this.getAttr("padding","1rem")}"><slot></slot></div>`}}a(jt,"observedAttributes",["max-width","padding","fluid"]);l("ui-container",jt);class Dt extends d{styles(){return":host{display:block}.flex{display:flex;flex-direction:var(--_direction,row);gap:var(--_gap,0);align-items:var(--_align,stretch);justify-content:var(--_justify,flex-start);flex-wrap:var(--_wrap,nowrap)}:host([inline]){display:inline-block}.flex{width:100%}"}template(){const e={start:"flex-start",end:"flex-end",between:"space-between",around:"space-around",evenly:"space-evenly"},t=(r,o)=>e[this.getAttr(r)]??this.getAttr(r,o);return`<div class="flex" style="--_direction:${this.getAttr("direction","row")};--_gap:${this.getAttr("gap","0")};--_align:${t("align","stretch")};--_justify:${t("justify","flex-start")};--_wrap:${this.getBoolAttr("wrap")?"wrap":"nowrap"}"><slot></slot></div>`}}a(Dt,"observedAttributes",["direction","gap","align","justify","wrap","inline"]);l("ui-flex",Dt);class qt extends d{styles(){return":host{display:block}.center{display:grid;place-items:center;min-height:var(--_min,auto)}:host([inline]){display:inline-block}"}template(){return`<div class="center" style="--_min:${this.getAttr("min-height","auto")}"><slot></slot></div>`}}a(qt,"observedAttributes",["inline","min-height"]);l("ui-center",qt);class Mt extends d{styles(){return":host{display:block;flex:1 1 auto}.spacer{min-width:var(--_size,0);min-height:var(--_size,0)}"}template(){return`<span class="spacer" style="--_size:${this.getAttr("size","0")}"></span>`}}a(Mt,"observedAttributes",["size"]);l("ui-spacer",Mt);class Pt extends d{styles(){return":host{display:block;width:100%}.frame{width:100%;aspect-ratio:var(--_ratio,16/9);overflow:hidden;position:relative}::slotted(*){width:100%;height:100%;object-fit:cover}"}template(){return`<div class="frame" style="--_ratio:${this.getAttr("ratio","16/9")}"><slot></slot></div>`}}a(Pt,"observedAttributes",["ratio"]);l("ui-aspect-ratio",Pt);class Ft extends d{styles(){return":host{display:block}.viewport{height:var(--_height,auto);max-height:var(--_max,20rem);overflow:auto;scrollbar-width:thin;scrollbar-color:var(--ui-color-border,#64748b) transparent}.viewport::-webkit-scrollbar{width:8px;height:8px}.viewport::-webkit-scrollbar-thumb{background:var(--ui-color-border,#64748b);border-radius:999px}"}template(){return`<div class="viewport" tabindex="0" style="--_height:${this.getAttr("height","auto")};--_max:${this.getAttr("max-height","20rem")}"><slot></slot></div>`}}a(Ft,"observedAttributes",["height","max-height","orientation"]);l("ui-scroll-area",Ft);class Ht extends d{styles(){return":host{display:block}.masonry{column-count:var(--_columns,3);column-gap:var(--_gap,1rem)}::slotted(*){break-inside:avoid;display:block;margin-bottom:var(--_gap,1rem)}@media(max-width:720px){.masonry{column-count:min(2,var(--_columns,3))}}"}template(){return`<div class="masonry" style="--_columns:${this.getNumAttr("columns",3)};--_gap:${this.getAttr("gap","1rem")}"><slot></slot></div>`}}a(Ht,"observedAttributes",["columns","gap"]);l("ui-masonry",Ht);class Wt extends d{constructor(){super(...arguments);a(this,"dragging",!1)}styles(){return":host{display:block;width:100%;height:100%}.split{display:grid;grid-template-columns:var(--_position,50%) .4rem 1fr;height:100%;min-height:10rem}:host([orientation=vertical]) .split{grid-template-columns:1fr;grid-template-rows:var(--_position,50%) .4rem 1fr}.separator{background:var(--ui-color-border,#cbd5e1);cursor:col-resize;touch-action:none}.separator:hover,.separator:focus{outline:0;background:var(--ui-color-primary,#4f46e5)}:host([orientation=vertical]) .separator{cursor:row-resize}.pane{min-width:0;min-height:0;overflow:auto}"}template(){return`<div class="split" style="--_position:${this.getNumAttr("position",50)}%"><div class="pane"><slot name="first"></slot></div><div class="separator" role="separator" tabindex="0" aria-valuenow="${this.getNumAttr("position",50)}" aria-valuemin="${this.getNumAttr("min",15)}" aria-valuemax="${this.getNumAttr("max",85)}"></div><div class="pane"><slot name="second"></slot></div></div>`}onRendered(){const t=this.$(".separator"),r=this.$(".split"),o=i=>{const n=Math.min(this.getNumAttr("max",85),Math.max(this.getNumAttr("min",15),i));this.setAttribute("position",String(n)),this.emit("ui-resize",{position:n})};t==null||t.addEventListener("pointerdown",i=>{this.dragging=!0,t.setPointerCapture(i.pointerId)}),t==null||t.addEventListener("pointermove",i=>{if(!this.dragging||!r)return;const n=r.getBoundingClientRect();o(this.getAttr("orientation")==="vertical"?(i.clientY-n.top)/n.height*100:(i.clientX-n.left)/n.width*100)}),t==null||t.addEventListener("pointerup",()=>{this.dragging=!1}),t==null||t.addEventListener("keydown",i=>{const n=i.key==="ArrowLeft"||i.key==="ArrowUp"?-2:i.key==="ArrowRight"||i.key==="ArrowDown"?2:0;n&&(i.preventDefault(),o(this.getNumAttr("position",50)+n))})}}a(Wt,"observedAttributes",["orientation","position","min","max"]);l("ui-split-pane",Wt);class Xt extends d{styles(){return':host{display:block;width:100%;height:100%}.viewport{display:grid;grid-template-columns:var(--_side,15rem) minmax(0,1fr) var(--_inspect,18rem);grid-template-rows:var(--_header,3.5rem) minmax(0,1fr) var(--_footer,2.5rem);grid-template-areas:"header header header" "sidebar main inspector" "footer footer footer";width:100%;height:100%;min-height:24rem}.header{grid-area:header}.sidebar{grid-area:sidebar;overflow:auto}.main{grid-area:main;overflow:auto;min-width:0}.inspector{grid-area:inspector;overflow:auto}.footer{grid-area:footer}.region{border:1px solid var(--ui-color-border,#cbd5e1);background:var(--ui-color-surface)}@media(max-width:900px){.viewport{grid-template-columns:minmax(0,1fr);grid-template-areas:"header" "main" "footer"}.sidebar,.inspector{display:none}}'}template(){return`<div class="viewport" style="--_side:${this.getAttr("sidebar-width","15rem")};--_inspect:${this.getAttr("inspector-width","18rem")};--_header:${this.getAttr("header-height","3.5rem")};--_footer:${this.getAttr("footer-height","2.5rem")}"><header class="region header"><slot name="header"></slot></header><aside class="region sidebar"><slot name="sidebar"></slot></aside><main class="region main"><slot></slot></main><aside class="region inspector"><slot name="inspector"></slot></aside><footer class="region footer"><slot name="footer"></slot></footer></div>`}}a(Xt,"observedAttributes",["sidebar-width","inspector-width","header-height","footer-height"]);l("ui-viewport-layout",Xt);class Vt extends d{styles(){return`
      :host { display: contents; }
    ...
    `}template(){return"<slot></slot>"}}a(Vt,"observedAttributes",["mode"]);l("ui-theme-provider",Vt);const w={version:1,query:"",filters:{},timeWindow:null,selection:null,compare:{enabled:!1},panels:{}};function k(s){return{...w,...s,filters:{...w.filters,...(s==null?void 0:s.filters)??{}},compare:{...w.compare,...(s==null?void 0:s.compare)??{}},panels:le(w.panels,s==null?void 0:s.panels)}}function ae(s,e){switch(e.type){case"setQuery":return{...s,query:e.query};case"setFilter":return{...s,filters:_(s.filters,e.key,e.value)};case"setPanelFilter":{const t=s.panels[e.panelId]??{};return{...s,panels:{...s.panels,[e.panelId]:{...t,filters:_(t.filters??{},e.key,e.value)}}}}case"setTimeWindow":return{...s,timeWindow:e.timeWindow?{...e.timeWindow}:null};case"setSelection":return{...s,selection:de(e.selection)};case"setCompare":return{...s,compare:{...s.compare,...e.compare}};case"patchPanelState":{const t=s.panels[e.panelId]??{},r={...t,...e.patch,filters:e.patch.filters?{...t.filters??{},...e.patch.filters}:t.filters};return{...s,panels:{...s.panels,[e.panelId]:r}}}case"restoreSavedView":return k(e.view);case"reset":return k(e.state);default:return s}}function ne(s){let e=k(s);const t=new Set,r=i=>{e=ae(e,i);for(const n of t)n(e)};return{getState(){return e},dispatch:r,subscribe(i){return t.add(i),()=>{t.delete(i)}},actions:{setQuery(i){r({type:"setQuery",query:i})},setFilter(i,n){r({type:"setFilter",key:i,value:n})},setPanelFilter(i,n,c){r({type:"setPanelFilter",panelId:i,key:n,value:c})},setTimeWindow(i){r({type:"setTimeWindow",timeWindow:i})},setSelection(i){r({type:"setSelection",selection:i})},setCompare(i){r({type:"setCompare",compare:i})},patchPanelState(i,n){r({type:"patchPanelState",panelId:i,patch:n})},restoreSavedView(i){r({type:"restoreSavedView",view:i})},reset(i){r({type:"reset",state:i})}}}}function _(s,e,t){const r={...s};return t===void 0?(delete r[e],r):(r[e]=t,r)}function le(s,e){var r;const t={};for(const[o,i]of Object.entries(s))t[o]={...i,filters:i.filters?{...i.filters}:void 0};for(const[o,i]of Object.entries(e??{}))t[o]={...t[o]??{},...i,filters:{...((r=t[o])==null?void 0:r.filters)??{},...i.filters??{}}};return t}function de(s){return s?{...s,meta:s.meta?{...s.meta}:void 0}:null}const Ot={background:"#0b1220",surface:"#101a2d",surfaceMuted:"#162238",border:"#22324d",text:"#e6edf7",textMuted:"#99abc7",accent:"#4c8dff",accentSoft:"rgba(76, 141, 255, 0.18)",positive:"#29b37d",warning:"#f2b84b",danger:"#ef6464",navWidth:"248px",inspectorWidth:"320px",topbarHeight:"72px",statusHeight:"36px",radius:"16px",gap:"16px",panelPadding:"16px",shadow:"0 18px 44px rgba(3, 10, 24, 0.28)",fontFamily:'"Segoe UI", Inter, "Helvetica Neue", Arial, sans-serif'},ce={background:"#f5f7fb",surface:"#ffffff",surfaceMuted:"#eef3fb",border:"#d8e2f1",text:"#122033",textMuted:"#5f718a",accent:"#2764ff",accentSoft:"rgba(39, 100, 255, 0.12)",positive:"#1f8b5f",warning:"#b8800e",danger:"#c53f3f",navWidth:"248px",inspectorWidth:"320px",topbarHeight:"72px",statusHeight:"36px",radius:"16px",gap:"16px",panelPadding:"16px",shadow:"0 18px 44px rgba(15, 23, 42, 0.08)",fontFamily:'"Segoe UI", Inter, "Helvetica Neue", Arial, sans-serif'},ue={...Ot,background:"#05070a",surface:"#0c1117",surfaceMuted:"#111923",border:"#93a4c2",text:"#ffffff",textMuted:"#c8d3e5",accent:"#7fb0ff",accentSoft:"rgba(127, 176, 255, 0.2)",positive:"#6ce3aa",warning:"#ffd56b",danger:"#ff9b9b"};function pe(s="dark",e){return{...s==="light"?ce:s==="high-contrast"?ue:Ot,...e}}function he(s){return{"--rlwb-bg":s.background,"--rlwb-surface":s.surface,"--rlwb-surface-muted":s.surfaceMuted,"--rlwb-border":s.border,"--rlwb-text":s.text,"--rlwb-text-muted":s.textMuted,"--rlwb-accent":s.accent,"--rlwb-accent-soft":s.accentSoft,"--rlwb-positive":s.positive,"--rlwb-warning":s.warning,"--rlwb-danger":s.danger,"--rlwb-nav-width":s.navWidth,"--rlwb-inspector-width":s.inspectorWidth,"--rlwb-topbar-height":s.topbarHeight,"--rlwb-status-height":s.statusHeight,"--rlwb-radius":s.radius,"--rlwb-gap":s.gap,"--rlwb-panel-padding":s.panelPadding,"--rlwb-shadow":s.shadow,"--rlwb-font-family":s.fontFamily}}const $=[{label:"1h",value:{preset:"1h",label:"Last hour"}},{label:"24h",value:{preset:"24h",label:"Last 24 hours"}},{label:"7d",value:{preset:"7d",label:"Last 7 days"}},{label:"30d",value:{preset:"30d",label:"Last 30 days"}}];function x(s,e){if(e)return e;const t=s.closest("ui-workbench-shell");return(t==null?void 0:t.workbenchStore)??(t==null?void 0:t.store)??null}function be(s,e){const t=he(pe(s,e));return Object.entries(t).map(([r,o])=>`${r}:${o}`).join(";")}function v(s){return s?s.assignedNodes({flatten:!0}).some(e=>e.nodeType===Node.ELEMENT_NODE?!0:e.nodeType===Node.TEXT_NODE?(e.textContent??"").trim().length>0:!1):!1}function f(s){return(s==null?"":String(s)).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function Ut(s,e){return Array.isArray(s)&&Array.isArray(e)?s.length!==e.length?!1:s.every((t,r)=>t===e[r]):s===e}function me(s,e){return Array.isArray(s)&&!Array.isArray(e)?s.includes(e):Ut(s,e)}function ge(s,e,t){if(!t)return Ut(s,e)?void 0:e;const r=Array.isArray(e)?e[0]:e,o=Array.isArray(s)?[...s]:s===void 0?[]:[s],i=o.findIndex(n=>n===r);return i>=0?o.splice(i,1):o.push(r),o.length>0?o:void 0}function ve(s,e){return!s&&!e?!0:!s||!e?!1:s.preset===e.preset&&s.from===e.from&&s.to===e.to&&s.timezone===e.timezone&&s.label===e.label}function fe(s){if(!s)return"";const e=[["Panel",s.panelId],["Entity",s.entityId],["Series",s.seriesId],["Point",s.pointId],["Label",s.label]];for(const[r,o]of Object.entries(s.meta??{}))e.push([r,typeof o=="object"?JSON.stringify(o):o]);const t=e.filter(([,r])=>r!=null&&String(r).trim().length>0).map(([r,o])=>`<dt>${f(r)}</dt><dd>${f(o)}</dd>`).join("");return t?`<dl class="rlwb-selection-list">${t}</dl>`:""}const m=`
  :host {
    display: block;
    box-sizing: border-box;
    min-width: 0;
...
`;class Yt extends d{constructor(){super(...arguments);a(this,"_store");a(this,"_theme");a(this,"_initialState");a(this,"hasNav",!1);a(this,"hasInspector",!1);a(this,"hasTopbar",!1);a(this,"hasStatus",!1)}get tone(){return this.getAttribute("tone")??"dark"}set tone(t){this.setAttribute("tone",t)}get theme(){return this._theme}set theme(t){this._theme=t,this.render()}get initialState(){return this._initialState}set initialState(t){this._initialState=t,this._store||this.render()}get store(){return this._store||(this._store=ne(this._initialState)),this._store}set store(t){this._store=t??void 0,this.render()}get workbenchStore(){return this.store}styles(){return m}template(){return`
      <div data-rl-workbench="" class="${["rlwb-shell",this.hasNav&&"rlwb-shell--has-nav",this.hasInspector&&"rlwb-shell--has-inspector"].filter(Boolean).join(" ")}" style="${be(this.tone,this._theme)}">
    ...
    `}onRendered(){const t=this.$('slot[name="nav"]'),r=this.$('slot[name="topbar"]'),o=this.$('slot[name="inspector"]'),i=this.$('slot[name="status"]'),n=()=>{const c=v(t),p=v(r),u=v(o),h=v(i);(c!==this.hasNav||p!==this.hasTopbar||u!==this.hasInspector||h!==this.hasStatus)&&(this.hasNav=c,this.hasTopbar=p,this.hasInspector=u,this.hasStatus=h,this.render())};t==null||t.addEventListener("slotchange",n),r==null||r.addEventListener("slotchange",n),o==null||o.addEventListener("slotchange",n),i==null||i.addEventListener("slotchange",n),n()}}a(Yt,"observedAttributes",["tone"]);l("ui-workbench-shell",Yt);class Kt extends d{styles(){return m}template(){const e=this.getAttribute("columns"),t=this.getAttribute("rows")??"",r=this.getAttribute("min-column-width"),o=r?`repeat(auto-fit, minmax(${r}, 1fr))`:e?/^\d+$/.test(e)?`repeat(${e}, 1fr)`:e:"repeat(auto-fit, minmax(320px, 1fr))";return`
      <div
    ...
    `}}a(Kt,"observedAttributes",["columns","rows","min-column-width","dense"]);l("ui-workbench-panel-layout",Kt);class Qt extends d{constructor(){super(...arguments);a(this,"_store");a(this,"unsubscribe");a(this,"localPanelState",{});a(this,"hasFooter",!1)}get store(){return this._store}set store(t){this._store=t,this.syncSubscription(),this.render()}onConnected(){this.syncSubscription()}onDisconnected(){var t;(t=this.unsubscribe)==null||t.call(this),this.unsubscribe=void 0}styles(){return m}template(){const t=this.getAttribute("title")??"",r=this.getAttribute("subtitle")??"",o=this.getAttribute("padding")??"md",i=this.getAttribute("tone")??"default",n=this.getPanelState(),c=this.hasAttribute("collapsible"),p=!!n.collapsed;return`
    ...
    `}onRendered(){const t=this.$('[data-action="toggle"]');t==null||t.addEventListener("click",()=>{const i=this.getAttribute("panel-id"),n=this.getPanelState(),c={collapsed:!n.collapsed},p=this.resolveStore();i&&p?p.actions.patchPanelState(i,c):(this.localPanelState={...n,...c},this.render())});const r=this.$('slot[name="footer"]'),o=()=>{const i=v(r);i!==this.hasFooter&&(this.hasFooter=i,this.render())};r==null||r.addEventListener("slotchange",o),o()}resolveStore(){return x(this,this._store)}getPanelState(){const t=this.getAttribute("panel-id"),r=this.resolveStore(),o=t&&r?r.getState().panels[t]:void 0;return{collapsed:this.hasAttribute("default-collapsed"),...this.localPanelState,...o}}syncSubscription(){var o;(o=this.unsubscribe)==null||o.call(this);const t=this.resolveStore(),r=this.getAttribute("panel-id");!t||!r||!this.isConnected||(this.unsubscribe=t.subscribe(()=>{this.render()}))}}a(Qt,"observedAttributes",["panel-id","title","subtitle","collapsible","default-collapsed","padding","tone"]);l("ui-workbench-panel",Qt);class Jt extends d{constructor(){super(...arguments);a(this,"_store");a(this,"unsubscribe")}get store(){return this._store}set store(t){this._store=t,this.syncSubscription(),this.render()}onConnected(){this.syncSubscription()}onDisconnected(){var t;(t=this.unsubscribe)==null||t.call(this),this.unsubscribe=void 0}styles(){return m}template(){var i;const t=this.getAttribute("label")??"Query",r=this.getAttribute("placeholder")??"Search, scope, or command",o=((i=this.resolveStore())==null?void 0:i.getState().query)??"";return`
    ...
    `}onRendered(){const t=this.$(".rlwb-query-input");t==null||t.addEventListener("input",r=>{var i;const o=r.currentTarget.value;(i=this.resolveStore())==null||i.actions.setQuery(o),this.emit("ui-query-change",{value:o})})}resolveStore(){return x(this,this._store)}syncSubscription(){var r;(r=this.unsubscribe)==null||r.call(this);const t=this.resolveStore();!t||!this.isConnected||(this.unsubscribe=t.subscribe(()=>{this.render()}))}}a(Jt,"observedAttributes",["label","placeholder"]);l("ui-workbench-query-bar",Jt);class xe extends d{constructor(){super(...arguments);a(this,"_store");a(this,"_filters",[]);a(this,"unsubscribe")}get store(){return this._store}set store(t){this._store=t,this.syncSubscription(),this.render()}get filters(){return this._filters}set filters(t){this._filters=t,this.render()}onConnected(){this.syncSubscription()}onDisconnected(){var t;(t=this.unsubscribe)==null||t.call(this),this.unsubscribe=void 0}styles(){return m}template(){var o;const t=(o=this.resolveStore())==null?void 0:o.getState();return`<div class="rlwb-filter-bar">${this._filters.map(i=>{var p,u;const n=i.scope==="panel"?(u=(p=t==null?void 0:t.panels[i.panelId??""])==null?void 0:p.filters)==null?void 0:u[i.key]:t==null?void 0:t.filters[i.key],c=i.options.map((h,b)=>`
            <button
              type="button"
              class="rlwb-filter-chip"
              data-filter-key="${f(i.key)}"
              data-filter-index="${b}"
              aria-pressed="${me(n,h.value)}"
            >
              ${f(h.label)}
            </button>
          `).join("");return`
        ...
        `}).join("")}</div>`}onRendered(){this.$$(".rlwb-filter-chip").forEach(t=>{t.addEventListener("click",()=>{var h,b;const r=t.dataset.filterKey??"",o=Number(t.dataset.filterIndex??"-1"),i=this._filters.find(g=>g.key===r),n=i==null?void 0:i.options[o],c=this.resolveStore();if(!i||!n||!c)return;const p=i.scope==="panel"?(b=(h=c.getState().panels[i.panelId??""])==null?void 0:h.filters)==null?void 0:b[i.key]:c.getState().filters[i.key],u=ge(p,n.value,!!i.multi);i.scope==="panel"&&i.panelId?c.actions.setPanelFilter(i.panelId,i.key,u):c.actions.setFilter(i.key,u),this.emit("ui-filter-change",{key:i.key,panelId:i.panelId,scope:i.scope??"global",value:u})})})}resolveStore(){return x(this,this._store)}syncSubscription(){var r;(r=this.unsubscribe)==null||r.call(this);const t=this.resolveStore();!t||!this.isConnected||(this.unsubscribe=t.subscribe(()=>{this.render()}))}}l("ui-workbench-filter-bar",xe);class ye extends d{constructor(){super(...arguments);a(this,"_store");a(this,"_options",$);a(this,"unsubscribe")}get store(){return this._store}set store(t){this._store=t,this.syncSubscription(),this.render()}get options(){return this._options}set options(t){this._options=t.length>0?t:$,this.render()}onConnected(){this.syncSubscription()}onDisconnected(){var t;(t=this.unsubscribe)==null||t.call(this),this.unsubscribe=void 0}styles(){return m}template(){var o;const t=(o=this.resolveStore())==null?void 0:o.getState().timeWindow;return`<div class="rlwb-time-range">${this._options.map((i,n)=>`
        <button
          type="button"
          class="rlwb-filter-chip"
          data-option-index="${n}"
          aria-pressed="${ve(t,i.value)}"
        >
          ${f(i.label)}
        </button>
      `).join("")}</div>`}onRendered(){this.$$(".rlwb-filter-chip").forEach(t=>{t.addEventListener("click",()=>{var i;const r=Number(t.dataset.optionIndex??"-1"),o=this._options[r];o&&((i=this.resolveStore())==null||i.actions.setTimeWindow(o.value),this.emit("ui-time-window-change",{value:o.value}))})})}resolveStore(){return x(this,this._store)}syncSubscription(){var r;(r=this.unsubscribe)==null||r.call(this);const t=this.resolveStore();!t||!this.isConnected||(this.unsubscribe=t.subscribe(()=>{this.render()}))}}l("ui-workbench-time-range-control",ye);class Gt extends d{constructor(){super(...arguments);a(this,"_store");a(this,"_renderContent");a(this,"unsubscribe");a(this,"hasActions",!1)}get store(){return this._store}set store(t){this._store=t,this.syncSubscription(),this.render()}get renderContent(){return this._renderContent}set renderContent(t){this._renderContent=t,this.render()}onConnected(){this.syncSubscription()}onDisconnected(){var t;(t=this.unsubscribe)==null||t.call(this),this.unsubscribe=void 0}styles(){return m}template(){var n;const t=this.getAttribute("title")??"Inspector",r=this.getAttribute("empty-state")??"Select a record, point, or entity to inspect it here.",o=((n=this.resolveStore())==null?void 0:n.getState().selection)??null,i=this._renderContent?"":o?fe(o):`<div class="rlwb-empty-state">${r}</div>`;return`
